...
class DAS():
    def __init__(self, mic_region, scan_region, z_source, fs, mic_pos, scan_resolution=0.15, mic_num=6, start_time=0, end_time=0.02, search_frel=800, search_freu=4000):
        ...
        #计算声源扫描平面的扫描点数
        self.n_scanpoints = ((self.scan_region[0][1]-self.scan_region[0][0])/self.scan_resolution+1)*((self.scan_region[0][3]-self.scan_region[0][2])/self.scan_resolution+1)
        self.n_scanpoints = int(self.n_scanpoints)
        #形成声源扫描平面的x、y坐标
        self.x = np.arange(self.scan_region[0][0], self.scan_region[0][1] + self.scan_resolution, self.scan_resolution)
        self.y = np.arange(self.scan_region[0][2], self.scan_region[0][3] + self.scan_resolution, self.scan_resolution)
        self.x_t = np.zeros([self.n_scanpoints, 3])
        #以下操作相当于将一个self.y*self.x的矩阵坐标拉成1*(self.x*self.y)的坐标
        #将x坐标沿水平方向复制len(self.y)次
        self.x_t[:, 0] = np.tile(self.x, [1, len(self.y)])
        #将y坐标轴坐标点数值每个复制len(self.x)次
        self.x_t[:, 1] = (self.y).repeat(len(self.x))
